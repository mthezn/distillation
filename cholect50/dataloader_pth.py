...
#        train_videos = sum([v for k,v in video_split.items() if k!=test_fold], []) if 'crossval' in dataset_variant else video_split['train']
    ...
    def transform(self):
        ...
        op_test   = [transforms.Resize((1024, 1024)), transforms.ToTensor(), ]
        op_train  = [transforms.Resize((1024, 1024))] + self.augmentation_list + [transforms.Resize((256, 448)), transforms.ToTensor()]
        if self.normalize:
